...
(
  (($# == 0)) && echo "Error: 請輸入至少一個參數" >&2 && return 1
  ...
  while [[ $1 ]]; do
    if [[ $1 == "-a" ]]; then
      allowOption=true
      shift
      [[ ! $1 ]] && echo "Error: 選項 -a 請輸入至少一個參數" >&2 && return 1
      continue
    fi
  ...
  for dir in "$disableDirs"; do
    ...
    # 如果參數是完整路徑，則直接檢查當前目錄是否匹配
    if ! [[ -d "$dir" ]]; then
      [[ $PWD == *$dir* ]] && echo "Error: 當前指令禁用於「當前目錄」" && return 1
